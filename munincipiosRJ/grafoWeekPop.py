...
for year in years:
    ...
    def nodeMarkerAndText(year, week):
        ...
        for node, adjacencies in enumerate(G.adjacency()):
            ...
            pop = G.nodes[node]['data'].populacao
            G.nodes[node]['data'].dengueDataCumulative += newData
            if newData != 0:
                newData = np.divide( newData, pop) * 100000

            G.nodes[node]['data'].logDengueDataCumulative += newData

            node_adjacencies.append(
                G.nodes[node]['data'].logDengueDataCumulative)
            node_text.append(f"Municipio: {G.nodes[node]['data'].city}[{str(node)}]  |Casos de Dengue por 100 mil: { str(G.nodes[node]['data'].logDengueDataCumulative) }")

        return node_adjacencies, node_text


    titleStr = f"Casos de dengue por 100 mil pessoas ao longo do ano de {year}."

...
def nodeMarkerAndText(year, week):
    ...
    for node, adjacencies in enumerate(G.adjacency()):
        ...
        node_adjacencies.append(
            G.nodes[node]['data'].totalYearDengueDataByPop[week])
        node_text.append(
            f"Municipio: {G.nodes[node]['data'].city}[{str(node)}]  |Casos de Dengue por 100 mil: { str(G.nodes[node]['data'].totalYearDengueDataByPop[week]) }")

    return node_adjacencies, node_text

titleStr = f"Casos de dengue por 100 mil pessoas."
figDict["layout"]["xaxis"] = dict(showgrid=False, zeroline=False, showticklabels=False)
figDict["layout"]["yaxis"] = dict(
...
for i in range(len(years)):
    nodeTraces[years[i]] = createNodeTrace(years[i], i)

figDict["data"] = [edge_trace, nodeTraces["2010"]]

for year in years:
    frame = dict(
        data=[edge_trace, nodeTraces[year]],
        name=str(year)
    )
